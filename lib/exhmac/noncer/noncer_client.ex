defmodule ExHmac.Noncer.Client do
  ...
  def check(nonce, curr_ts, ttl, precision) do
    with(
      {arrived_at, raw_result, result} <- check_call(nonce, curr_ts, ttl, precision),
  ...
  def check_call(nonce, curr_ts, ttl, precision) do
    GenServer.call(Server, {nonce, curr_ts, ttl, precision})
  end
